...
def predict_perf_model(data, model_type=3):
    ...
    pickled_model = pickle.load(open("/home/udits/githubrepos/heterogenous-scaling-in-k8s/spark/experiments/scripts/model/" + model_file_name, "rb"))
    prediction = np.floor(pickled_model.predict(data))
    mean_prediction = np.mean(prediction, axis=0)
    prediction = np.ceil(mean_prediction)
