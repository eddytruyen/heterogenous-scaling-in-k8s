...
def generate_matrix(initial_conf, adaptive_scalers, runtime_manager, namespace, tenants, completion_time, previous_tenants, previous_conf, total_cpu, total_memory, ignore_auto_scaler):
        ...
        def process_samples(rm,tenant_nb):
                        ...
                        results=[]
                        nr_of_experiments=len(next_exp)
                        print("Processing samples " + str(nr_of_experiments) + " experiments")
                        for i,ws in enumerate(next_exp):
        ...
        def is_non_monotonic(scalingFunction, resource_cost_conf, conf_completion_time, conf_tenant_nb, resource_cost_historic, historic_completion_time, historic_tenant_nb,monotonic_threshold,slo):
            if conf_completion_time <= slo:
                return False
            print("Comparing with historic completion time: " + str(historic_completion_time))
            if conf_tenant_nb != historic_tenant_nb:
                print("Comparing with larger tenant_nb")
                return resource_cost_conf >= resource_cost_historic and conf_completion_time > historic_completion_time + (slo * monotonic_threshold)
            else:
                #print("Comparing with same tenant_nb")
                #a=resource_cost_conf-scalingFunction.targetAbsoluteTotalCost(conf_completion_time, tenant_nb)
                #print(str(conf_completion_time) + ", " + str(resource_cost_conf) +  " -> " + str(a))  
                #b=resource_cost_historic-scalingFunction.targetAbsoluteTotalCost(historic_completion_time, tenant_nb)
                #print(str(historic_completion_time) + ", " + str(resource_cost_historic) + " -> " + str(b))
                #print(str(a) + "-abs(" + str(b) + ") >= " + str(monotonic_treshold) + "*abs("+ str(b) + ") ??")
        ...
        if not ignore_auto_scaler and len(previous_conf)==len(alphabet['elements']) and int(previous_tenants) > 0 and float(completion_time) > 0:
            ...
            if not (mono_constraint_violated or positive_outlier):
                total_rc=total_resource_cost(adaptive_scaler.workers,previous_conf,cost_aware=False)
                adaptive_scaler.ScalingFunction.update(tenant_nb,total_rc, float(completion_time), {"cpu":0.5, "memory":0.5} if tenant_nb > 2 else {"cpu":0.8, "memory":0.2} )
            #if not outlier:     
       ...
       # else:
        ...
        while tenant_nb <= maxTenants and evaluate:
            ...
            if not no_exps and adaptive_scaler.ScalingDownPhase and adaptive_scaler.StartScalingDown:
                ...
                intermediate_result=find_optimal_results(runtime_manager,tenant_nb,tmp_adaptive_scaler.workers,results,slo)[0]
                tipped_over_intermediate_confs=return_failed_confs(runtime_manager,tenant_nb,tmp_adaptive_scaler.workers, results, lambda r: float(r['CompletionTime']) > slo and r['Successfull'] == 'true' and float(r['CompletionTime']) <= slo * scaling_up_threshold)
                print("Current_tipped_over_intermediate_confs")
                print(tipped_over_intermediate_confs)
                intermediate_states=tmp_adaptive_scaler.validate_result(intermediate_result, get_conf(tmp_adaptive_scaler.workers,intermediate_result), slo)
                intermediate_state=intermediate_states.pop(0)
                ...
                if not (intermediate_states and intermediate_states.pop(0) == UNDO_SCALE_ACTION):
                    ...
                    start=remove_failed_confs(runtime_manager, tenant_nb, lst, tmp_adaptive_scaler.workers, rm, results, slo, get_conf(adaptive_scaler.workers, intermediate_result), start, adaptive_window.get_current_window(),results[0],[],scaling_up_threshold, sampling_ratio, intermediate_remove=False, careful_scaling=adaptive_scaler.careful_scaling, positive_outlier=positive_outlier)
...
def remove_failed_confs(runtime_manager, tenant_nb, sorted_combinations, workers, rm, results, slo, optimal_conf, start, window, previous_result, tipped_over_results, scaling_up_threshold, sampling_ratio, startingTenant=False, intermediate_remove=False, higher_tenant_remove=False, careful_scaling=False, tenant_nb_workers=[], positive_outlier=False):
		...
		#					print("Removing config because it has a higher resource cost than the current optimal result and therefore this config and all higher configs are not cost effective for the lower number of tenants")
...
def resource_cost(workers, conf, cost_aware=True, res=None):
        cost=0
        for w,c in zip(workers,conf):
            worker_cost=0
            for resource_name in w.resources.keys():
                if not res or (res and resource_name == res):
...
def total_resource_cost(workers, conf, cost_aware=True):
        resources=dict()
        for resource_name in workers[0].resources.keys():
            resources[resource_name]=resource_cost(workers,conf,cost_aware,res=resource_name)
        return resources
