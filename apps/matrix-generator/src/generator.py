                                    ...
                                    def process_states(conf_and_states, original_adaptive_scaler=None):
                                        ...
                                        if state == RETRY_WITH_ANOTHER_WORKER_CONFIGURATION:
                                            ...
                                            if adaptive_scaler.ScalingDownPhase:
                                                    ...
                                                    try:
                                                        start_and_window=filter_samples(adaptive_scalers,lst,adaptive_scaler,start, window, previous_tenant_results, 1, tenant_nb, slo, True, adaptive_scaler.ScaledWorkerIndex, log=LOG_FILTERING, original_adaptive_scaler=original_adaptive_scaler, initial_conf=opt_conf)
                                                        ...
                                                        return process_states([[],adaptive_scaler.find_cost_effective_config(opt_conf, slo, tenant_nb, scale_down=True, only_failed_results=only_failed_results)], original_adaptive_scaler=original_adaptive_scaler, initial_conf=opt_conf)
                                                    ...
                                                    print([utils.array_to_str(el) for el in lst])
                                                    try:
                                                        start_and_window=filter_samples(adaptive_scalers,lst,adaptive_scaler, lst.index(scaled_conf), 1, previous_tenant_results, 1, tenant_nb, slo, True, adaptive_scaler.ScaledWorkerIndex, log=LOG_FILTERING)
                                                        print("Starting at index " + str(start_and_window[0]) + " with window " +  str(start_and_window[1]))
                                                        print([utils.array_to_str(el) for el in lst])
                                                        scaled_conf=lst[start_and_window[0]]
                                                        adaptive_scaler=add_incremental_result(adaptive_scalers, tenant_nb,d,sla,adaptive_scaler,slo, lambda x, slo: True, previous_conf=previous_conf,next_conf=scaled_conf)
	...
	while tenant_nb <= maxTenants and evaluate:
		...
		if state == NO_COST_EFFECTIVE_RESULT:
			...
			if states and states.pop(0) == UNDO_SCALE_ACTION:
				...
				conf_array=sort_configs(adaptive_scaler.workers,rm.get_left_over_configs())#sort_configs(adaptive_scaler.workers,rm.get_left_over_configs())
				reset_adaptive_scaler=True
				if conf_array:
                                        reset_adaptive_scaler=False
...
def filter_samples(adaptive_scalers,sorted_combinations, adaptive_scaler, start, window, previous_results, start_tenant, tenant_nb, slo, check_workers=False, ScaledDownWorkerIndex=-1, log=True, original_adaptive_scaler=None, initial_conf=[]):
        i=start_tenant
        new_window=window
        start_window=window
        if previous_results:
                                ...
                                for el in range(start, start+window):
                                        ...
                                        if (check_workers and i < tenant_nb and all_flagged_conf(adaptive_scaler.workers, sorted_combinations[el-(window-new_window)])) or (cost > MAXIMUM_TRANSITION_COST or nb_shrd_replicas < minimum_shared_replicas) or not (not check_workers or involves_worker(adaptive_scaler.workers, sorted_combinations[el-(window-new_window)], ScaledDownWorkerIndex)) or (original_adaptive_scaler and check_workers and resource_cost(original_adaptive_scaler, initial_conf) < resource_cost(adaptive_scaler, sorted_combinations[el-(window-new_window)])) or (i > tenant_nb and tenant_nb_X_result_conf_conflict_with_higher_tenants(adaptive_scalers,previous_results, adaptive_scaler, tenant_nb, sorted_combinations[el-(window-new_window)], slo)):
                                            ...
                                            if log:
                                                print("not removed")
                                if new_window == 0:
                                        return filter_samples(adaptive_scalers, sorted_combinations, adaptive_scaler, start+start_window, start_window, previous_results, start_tenant, tenant_nb, slo, check_workers, ScaledDownWorkerIndex, log, original_adaptive_scaler, initial_conf)
                                else:
                                        i+=1
                                        if log:
                ...
                for el in range(start, start+window):
                        ...
                        if log:
                            print(result_conf)
                        if not (not check_workers or involves_worker(adaptive_scaler.workers, result_conf, ScaledDownWorkerIndex)) or (original_adaptive_scaler and check_workers and resource_cost(original_adaptive_scaler, initial_conf) < resource_cost(adaptive_scaler, result_conf)):
                                if log and ScaledDownWorkerIndex > -1:
                                    print("SCALING INDEX = " + str(ScaledDownWorkerIndex))
                                if log:
                                    print("removed")
                                ...
                                if log:
                                    print("not removed")
                if new_window == 0:
                        return filter_samples(adaptive_scalers, sorted_combinations, adaptive_scaler, start+window, window, previous_results, start_tenant, tenant_nb, slo, check_workers, ScaledDownWorkerIndex, log,original_adaptive_scaler,initial_conf)
